...
class GeocodeInsarArgparse(YamlArgparse):
    ...
    def __init__(self):
        ...
        self.parser.add_argument('--rslc-h5', dest='rslc_h5', type=str,
                help='Path to reference RSLC HDF5 used for interferogram formation. Needed for metadata.')
        self.parser.add_argument('--runw-h5', dest='runw_h5', type=str,
                help='Path to input RUNW HDF5 to be geocoded. Needed for unwrapped rasters.')
        ...
        self.parser.add_argument('--azimuth-looks', dest='azimuth_looks', type=int, default=1,
                help='Azimuth looks of the interferogram in the RUNW product. Must be odd integer.')
        self.parser.add_argument('--range-looks', dest='range_looks', type=int, default=1,
                help='Range looks of the interferogram in the RUNW product. Must be odd integer.')
        self.parser.add_argument('--interp', dest='interp_method', type=str, default='BILINEAR',
                help='DEM interpolation method. Available methods BILINEAR (default), BICUBIC, NEAREST and BIQUINTIC.')
        self.parser.add_argument('--no-connected-components', action='store_true', default=False,
                help='Disable geocoded connectedComponents dataset. False by default.')
        self.parser.add_argument('--no-phase-sigma', action='store_true', default=False,
                help='Disable geocoded phaseSigmaCoherence dataset. False by default.')
        self.parser.add_argument('--no-unwrapped-phase', action='store_true', default=False,
                help='Disable geocoded unwrappedPhase dataset. False by default.')
