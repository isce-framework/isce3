...
from nisar.workflows.filter_data import filter_data, get_raster_block, get_raster_info, block_param_generator, write_raster_block
...
class IonosphereEstimation:
    ...
    def compute_disp_nondisp(self, 
        ...
        if self.diversity_method == 'split_main_band':
            if (phi_sub_low is None) or (phi_sub_high is None):
                err_str = f"sub-band unwrapped interferograms are required"\
                    " for split_main_band method."
                error_channel.log(err_str)
                raise ValueError(err_str)
    ...
    def get_mask_array(self, 
        ...
        if side_array is not None:
            ...
            main_array = resample_freqA_array(
                slant_main, 
                slant_side, 
                main_array)
            if low_band_array is not None:
                low_band_array = resample_freqA_array(
                    slant_main, 
                    slant_side, 
                    low_band_array)
            if high_band_array is not None:
                high_band_array = resample_freqA_array(
                    slant_main, 
                    slant_side, 
                    high_band_array)
    ...
    def estimate_iono_std(
        ...
        if (side_coh is not None) and (resample_flag):
            ...
            main_coh = resample_freqA_array(
                slant_main, 
                slant_side, 
                main_coh)
            if low_band_coh is not None:
                low_band_coh = resample_freqA_array(
                    slant_main, 
                    slant_side, 
                    low_band_coh)
            if high_band_coh is not None:
                high_band_coh = resample_freqA_array(
                    slant_main, 
                    slant_side, 
                    high_band_coh)
    ...
    def compute_unwrapp_error(
        ...
        if side_runw is not None:
            main_runw = resample_freqA_array(
                self.slant_main, 
                self.slant_side, 
                main_runw)

            if low_sub_runw is not None:
                low_sub_runw = resample_freqA_array(
                    self.slant_main, 
                    self.slant_side, 
                    low_sub_runw)

            if high_sub_runw is not None:
                high_sub_runw = resample_freqA_array(
                    slant_main, 
                    slant_side, 
                    high_sub_runw)
...
def resample_freqA_array(slant_main, 
                        slant_side, 
                        target_runw):
    ...
    def low_pass_filter(self, 
        ...
        for iter_cnt in range(self.iteration):
            ...
            for block_param in block_params:
                if iter_cnt == 0:
                    data_block = read_iono_array(data_str, block_param)
                    data_sig_block = read_iono_array(data_sig_str, block_param)
                    mask_block = read_iono_array(mask_str, block_param)
                else: 
                    data_block = read_iono_array(output_str, block_param)
                    data_sig_block = read_iono_array(output_sig_str, block_param)
                    mask_block = read_iono_array(mask_str, block_param)
                    # Replace the valid pixels with original unfiltered data
                    # to avoid too much smoothed signal
                    unfilt_data_block = read_iono_array(data_str, block_param)
                ...
                if self.filling_method == "smoothed":
                    filled_data = self.fill_with_smoothed(data_block)
                    filled_data_sig = self.fill_with_smoothed(data_sig_block) 

                elif self.filling_method == "nearest":
                    filled_data = self.fill_nearest(data_block)
                    filled_data_sig = self.fill_nearest(data_sig_block)
                ...
                # write_raster_block(output_str, filt_data, block_param)
                write_array(output_str, filt_data,  block_row=block_param.write_start_line)
                # write_raster_block(output_sig_str, filt_data_sig, block_param)
                write_array(output_sig_str, filt_data_sig,  block_row=block_param.write_start_line)
...
def read_iono_array(raster, block_param):
