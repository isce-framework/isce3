...
def get_output_geo_grid(
    ...
    top_left : dict
        ...
        `epsg` argument. A dict containing 'x' and 'y' keys. If either value in the dict
    ...
    bottom_right : dict
        ...
        'x' and 'y' keys. If either value in the dict is `None`, it is inferred from the
    ...
    epsg : int or None
        ...
        `dem_raster` is used.
    dem_raster : isce3.io.Raster
        The input digital elevation model (DEM) raster dataset. If any other parameters
        are not provided, they are inferred from the raster.
    ...
    epsg = get_option(epsg, default=dem_raster.get_epsg())
