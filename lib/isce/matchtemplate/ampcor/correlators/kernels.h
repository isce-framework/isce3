...
// code guard
#if !defined(ampcor_libampcor_correlators_kernels_h)
#define ampcor_libampcor_correlators_kernels_h
...
namespace ampcor {
    namespace kernels {
        ...
        auto c2r(const std::complex<float> * scratch,
                 std::size_t pairs, std::size_t zmdDim
                 ) -> float *;

        // assemble the offset field
        void offsetField(const int * zoomed,
                         std::size_t pairs,
