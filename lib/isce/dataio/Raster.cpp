...
namespace isce {
  namespace dataio {
    ...
    Raster::Raster() : dataset(nullptr), _readonly(true) {
      /*
    ...
    Raster::Raster(const std::string &fname, bool readOnly=true) : _readonly(readOnly) {
      /*
      ...
      if (dataset == nullptr) {
        std::string errstr = "In Raster::Raster() - Cannot open file '";
        errstr += fname;
        errstr += "'.";
    ...
    Raster::Raster(const Raster &rast) : dataset(rast.dataset), _readonly(rast._readonly) {
      /*
    ...
    void Raster::loadFrom(const std::string &fname, bool readOnly=true) {
      /*
       *  Provides the same functionality as Raster::Raster(std::string&,bool), but assumes that the
      ...
      // Note that in most cases if GDALOpen fails it will throw a CPL_ERROR anyways
      if (dataset == nullptr) {
        std::string errstr = "In Raster::loadFrom() - Cannot open file '";
        errstr += fname;
        errstr += "'.";
        throw std::runtime_error(errstr);
      }
