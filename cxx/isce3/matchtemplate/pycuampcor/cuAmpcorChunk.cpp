...
{
    // All of these throws check for features of this CPU ampcor port
    // which are not yet supported. Support for these features is
    ...
    loadReferenceChunk();
    // take amplitudes
    cuArraysAbs(c_referenceBatchRaw, r_referenceBatchRaw);

#ifdef CUAMPCOR_DEBUG
    // dump the raw reference image(s)
    c_referenceBatchRaw->outputToFile("c_referenceBatchRaw");
    r_referenceBatchRaw->outputToFile("r_referenceBatchRaw");
#endif

    // compute and subtract mean values (for normalized)
    cuArraysSubtractMean(r_referenceBatchRaw);

#ifdef CUAMPCOR_DEBUG
    // dump the raw reference image(s)
    r_referenceBatchRaw->outputToFile("r_referenceBatchRawSubMean");
#endif

    // load secondary image chunk
    loadSecondaryChunk();
    // take amplitudes
    cuArraysAbs(c_secondaryBatchRaw, r_secondaryBatchRaw);

#ifdef CUAMPCOR_DEBUG
    // dump the raw secondary image(s)
    c_secondaryBatchRaw->outputToFile("c_secondaryBatchRaw");
    r_secondaryBatchRaw->outputToFile("r_secondaryBatchRaw");
#endif
    ...
    if(param->algorithm == 0) {
        ...
        cuCorrTimeDomain(r_referenceBatchRaw, r_secondaryBatchRaw, r_corrBatchRaw);
    }

#ifdef CUAMPCOR_DEBUG
    // dump the un-normalized correlation surface
    r_corrBatchRaw->outputToFile("r_corrBatchRawUnNorm");
#endif

    // normalize the correlation surface
    corrNormalizerRaw->execute(r_corrBatchRaw, r_referenceBatchRaw, r_secondaryBatchRaw);

#ifdef CUAMPCOR_DEBUG
    // dump the normalized correlation surface
    r_corrBatchRaw->outputToFile("r_corrBatchRaw");
#endif
    ...
    cuArraysCopyExtractCorr(r_corrBatchRaw, r_corrBatchRawZoomIn, i_corrBatchZoomInValid, offsetInit);
    ...
    i_corrBatchValidCount->outputToFile("i_corrBatchValidCount");
#endif

    // step3: divide the peak value by the mean of surrounding values
    cuEstimateSnr(r_corrBatchSum, i_corrBatchValidCount, r_maxval, r_snrValue);

#ifdef CUAMPCOR_DEBUG
    offsetInit->outputToFile("i_offsetInit");
    r_snrValue->outputToFile("r_snrValue");
    r_covValue->outputToFile("r_covValue");
#endif
...
#ifdef CUAMPCOR_DEBUG
    offsetInit->outputToFile("i_offsetInitAdjusted");
    ...
    c_referenceBatchOverSampled->outputToFile("c_referenceBatchOverSampled");
    r_referenceBatchOverSampled->outputToFile("r_referenceBatchOverSampled");
#endif

    // compute and subtract the mean value
    cuArraysSubtractMean(r_referenceBatchOverSampled);
    ...
    if(param->algorithm == 0) {
        ...
        cuCorrTimeDomain(r_referenceBatchOverSampled, r_secondaryBatchOverSampled, r_corrBatchZoomIn);
    }

#ifdef CUAMPCOR_DEBUG
    // dump the oversampled correlation surface (un-normalized)
    r_corrBatchZoomIn->outputToFile("r_corrBatchZoomInUnNorm");
#endif

    // normalize the correlation surface
    corrNormalizerOverSampled->execute(r_corrBatchZoomIn, r_referenceBatchOverSampled, r_secondaryBatchOverSampled);

#ifdef CUAMPCOR_DEBUG
    // dump the oversampled correlation surface (normalized)
    r_corrBatchZoomIn->outputToFile("r_corrBatchZoomIn");
#endif

    // remove the last row and col to get even sequences
    cuArraysCopyExtract(r_corrBatchZoomIn, r_corrBatchZoomInAdjust, make_int2(0,0));

#ifdef CUAMPCOR_DEBUG
    // dump the adjusted correlation Surface
    r_corrBatchZoomInAdjust->outputToFile("r_corrBatchZoomInAdjust");
#endif
    ...
    r_corrBatchZoomInOverSampled->outputToFile("r_corrBatchZoomInOverSampled");
#endif

    //find the max again
    cuArraysMaxloc2D(r_corrBatchZoomInOverSampled, offsetZoomIn, corrMaxValue);

#ifdef CUAMPCOR_DEBUG
    // dump the max location on oversampled correlation surface
    offsetZoomIn->outputToFile("i_offsetZoomIn");
    corrMaxValue->outputToFile("r_maxvalZoomInOversampled");
#endif
    ...
    cuArraysCopyInsert(offsetFinal, offsetImage, idxDown_*param->numberWindowDownInChunk, idxAcross_*param->numberWindowAcrossInChunk);
    // snr
...
void cuAmpcorChunk::loadReferenceChunk()
{
    ...
    ChunkOffsetAcross->copyToDevice();
    ...
    if(referenceImage->isComplex())
    {
        ...
        referenceImage->loadToDevice((void *)c_referenceChunkRaw->devData, startD, startA, height, width);
        ...
        if(param->derampMethod == 0) {
            cuArraysCopyToBatchAbsWithOffset(c_referenceChunkRaw, param->referenceChunkWidth[idxChunk],
                c_referenceBatchRaw, ChunkOffsetDown->devData, ChunkOffsetAcross->devData);
        }
        else {
    ...
    // if the image is real
        ...
        throw std::runtime_error("real images not supported");
    }
    ...
    if(secondaryImage->isComplex())
    {
        ...
        if(param->derampMethod == 0) {
            cuArraysCopyToBatchAbsWithOffset(c_secondaryChunkRaw, param->secondaryChunkWidth[idxChunk],
                c_secondaryBatchRaw, ChunkOffsetDown->devData, ChunkOffsetAcross->devData);
        }
        else {
...

{
    ...
    referenceBatchOverSampler = new cuOverSamplerC2C(
        c_referenceBatchRaw, c_referenceBatchOverSampled,
        ...
        c_referenceBatchRaw->count);

    secondaryBatchOverSampler = new cuOverSamplerC2C(
        c_secondaryBatchZoomIn, c_secondaryBatchOverSampled,
        c_secondaryBatchZoomIn->height, c_secondaryBatchZoomIn->width,
        c_secondaryBatchOverSampled->height, c_secondaryBatchOverSampled->width,
        c_secondaryBatchRaw->count);
