 ...
 * < operator evaluation.
 *
 * \param[out]  bool    Whether or not left double is less than right double with
 *                      mask values taken into account. If both sides not masked,
 *                      return left double < right double. If only right masked,
 *                      return true. Other cases return false.
 * \param[in]   lhs     Left double_mask tuple
 * \param[in]   rhs     Right double_mask tuple
 */
struct masked_min_compare {
    __host__ __device__
    bool operator()(const double_mask lhs, const double_mask rhs) {
...
masked_minmax(const thrust::device_vector<double> & data,
    ...
    if (data.size() != mask.size())
    {
        throw isce3::except::LengthError(ISCE_SRCINFO(),
                "Data and mask vectors have different sizes");
    }
    ...
    auto masked_begin = thrust::make_zip_iterator(thrust::make_tuple(data.cbegin(),
                                                                     mask.cbegin()));
    auto masked_end = thrust::make_zip_iterator(thrust::make_tuple(data.cend(),
                                                                   mask.cend()));

    auto data_min = thrust::min_element(thrust::device, masked_begin, masked_end,
                                        masked_min_compare());
    auto data_max = thrust::max_element(thrust::device, masked_begin, masked_end,
                                        masked_max_compare());

    return thrust::pair<double, double>(thrust::get<0>(data_min[0]),
                                        thrust::get<0>(data_max[0]));
}
