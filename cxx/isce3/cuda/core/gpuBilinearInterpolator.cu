...
template <class U>
__host__ void isce3::cuda::core::gpuBilinearInterpolator<U>::interpolate_h(const Matrix<double>& truth, Matrix<U>& m, double start, double delta, U* h_z) {
    /*
...
template <class U>
__device__ U isce3::cuda::core::gpuBilinearInterpolator<U>::interpolate(double x, double y, const U* z, size_t nx, size_t ny=0) {
    size_t x1 = floor(x);
    size_t x2 = ceil(x);
    size_t y1 = floor(y);
    size_t y2 = ceil(y);
