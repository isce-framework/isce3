...
from pybind_nisar.workflows import unwrap, h5_prep
...
def test_unwrap_run():
    ...
    with h5py.File(out_paths['RIFG'], "a", libver='latest', swmr=True) as h_rifg:
        ds = h_rifg[os.path.join(product_path, 'wrappedPhase')]
        ...
        h_rifg[os.path.join(product_path, 'wrappedPhase')][:, :] = ref_igram
        h_rifg[os.path.join(product_path, 'phaseSigmaCoherence')][:, :] = ref_coh
        h_rifg.close()
...
def test_unwrap_validate():
    ...
    with h5py.File(os.path.join(scratch_path, 'runw.h5'), 'r') as h_runw:
        ...
        ref_igram = h_rifg[os.path.join(group_path.replace('RUNW', 'RIFG'), 'wrappedPhase')]
