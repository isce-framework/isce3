...
def test_ampcor():
    ...
    for impl in impls:
        ...
        datadir = os.path.join(iscetest.data, "ampcor", "accuracy-testdata",
                               "ovs128-rho0.8")
        ...
        margin = 0
        margin_rg = 2 * margin + 2*ampcor.halfSearchRangeAcross + ampcor.windowSizeWidth
        margin_az = 2 * margin + 2*ampcor.halfSearchRangeDown + ampcor.windowSizeHeight
        ...
        ampcor.corrSurfaceOverSamplingMethod = 0  # fft
        ...
        create_empty_dataset('dense_offsets',
                             ampcor.numberWindowAcross,
                             ampcor.numberWindowDown, 2, gdal.GDT_Float32)
        create_empty_dataset('gross_offsets',
                             ampcor.numberWindowAcross,
                             ampcor.numberWindowDown, 2, gdal.GDT_Float32)
        create_empty_dataset('snr',
                             ampcor.numberWindowAcross,
                             ampcor.numberWindowDown, 1, gdal.GDT_Float32)
        create_empty_dataset('covariance',
                             ampcor.numberWindowAcross,
        ...
        for fname in ("covariance", "dense_offsets", "gross_offsets", "snr"):
            ...
            for i in range(len(got)):
                if abs(got[i] - expected[i]) > tol:
                    print("got", got[i], "but expected", expected[i], "diff is", abs(got[i] - expected[i]))
                    print("at index", i)
                    assert False
