...
{
    typedef isce::orbit_wip::Orbit Orbit;
...
{
    isce::orbit_wip::Orbit orbit (refepoch, spacing, size);
...
TEST_F(OrbitTest, OrbitView)
{
    ...
    isce::orbit_wip::OrbitView view = orbit;

    EXPECT_EQ( orbit.refepoch(), view.refepoch() );
    EXPECT_EQ( orbit.time(), view.time() );
    EXPECT_EQ( orbit.position().data(), view.position() );
    EXPECT_EQ( orbit.velocity().data(), view.velocity() );
}
...
{
    isce::orbit_wip::Orbit orbit (refepoch, spacing);
...
{
    {
        isce::orbit_wip::Orbit orbit (refepoch, spacing, 0);
...
TEST_F(OrbitTest, Subinterval)
{
    ...
    isce::orbit_wip::OrbitView view = orbit.subinterval(start, stop);
    ...
    EXPECT_EQ( view.position(), &orbit.position()[start] );
    EXPECT_EQ( view.velocity(), &orbit.velocity()[start] );
}
