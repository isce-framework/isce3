...
TEST_F(OrbitTest, FromHostOrbit)
{
    HostOrbit h_orbit(statevecs);
    DeviceOrbit d_orbit(h_orbit);

    EXPECT_EQ( d_orbit.referenceEpoch(), h_orbit.referenceEpoch() );
    EXPECT_EQ( d_orbit.interpMethod(), h_orbit.interpMethod() );
    EXPECT_EQ( d_orbit.time(), h_orbit.time() );

    for (int i = 0; i < h_orbit.size(); ++i) {
        EXPECT_EQ( d_orbit.position(i), h_orbit.position(i) );
        EXPECT_EQ( d_orbit.velocity(i), h_orbit.velocity(i) );
    }
}

TEST_F(OrbitTest, Constructor)
{
    DeviceOrbit orbit(statevecs);
...
{
    DeviceOrbit orbit1(statevecs);
    DeviceOrbit orbit2(statevecs);
    DeviceOrbit orbit3;
...
{
    DeviceOrbit orbit(statevecs, OrbitInterpMethod::Hermite);
...
{
    DeviceOrbit orbit(statevecs, OrbitInterpMethod::Legendre);
