...
double _stdev(std::vector<double>& v)
{
    double m = _mean(v);
    double accum = 0.0;
...
std::complex<double> TestSignal::eval(double t)
{
...
protected:
    ...
    void fill_ref(const std::vector<double>& times)
    {
        auto nt = times.size();
        ref.assign(nt, 0.0);
        for (int i = 0; i < nt; ++i) {
    ...
    void mask_edges(const std::vector<double>& times)
    {
        auto nt = times.size();
        for (int i = 0; i < nt; ++i) {
            auto t = times[i];
    ...
    std::vector<double> gen_rand_times()
    {
