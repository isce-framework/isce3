...
TYPED_TEST(IH5Test, nochunk) {
    ...
    //Create a HDF5 file
    std::string wfilename = "castread_" + std::string(typeid(FirstParam).name()) 
    ...
    isce3::io::IDataSet dset = grp.createDataSet(std::string("data"), _inmatrix.data(), shp); 
    {
        ...
        ASSERT_EQ( img.dtype(1), isce3::io::asGDT<FirstParam>);

        //Read data type with casting into another matrix
        //And compute check sum
        int hsum;
        {
            isce3::core::Matrix<SecondParam> _readmatrix(length, width);
            img.getBlock(_readmatrix, 0, 0, 1);

            isce3::io::Raster readRaster(_readmatrix);
            ASSERT_EQ( readRaster.dtype(1), isce3::io::asGDT<SecondParam>);
...
TYPED_TEST(IH5Test, chunk) {
    ...
    isce3::io::IGroup grp = fic.openGroup("/");
    std::array<int,2> shp={length, width};
    isce3::io::IDataSet dset = grp.createDataSet<FirstParam>(std::string("data"), shp, 1);
