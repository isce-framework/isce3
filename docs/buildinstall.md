...
If you need to develop on ISCE3, you will need to build ISCE3 from source using CMake.
...
You will need to add the built pythn packages / extensions to your PYTHONPATH, and also add the ISCE3 C++ library to your library loader path.
...
macOS:
```bash
export DYLD_FALLBACK_LIBRARY_PATH=$(realpath install/lib*)
export PYTHONPATH=$PYTHONPATH:$(realpath install/packages)
```
